module.exports = function({ api, modules, config, __GLOBAL, User, Thread, Rank, Economy, Fishing, Nsfw }) {
	...
	/* ================ Staff Commands ==================== */
		...
		if (__GLOBAL.threadBlocked.includes(threadID)) {
			if (contentMessage == `${prefix}unbox` && admins.includes(senderID)) {
				...
				Thread.unban(threadID).then(success => {
					...
					api.sendMessage("Đã bỏ chặn hoạt động bot ở nhóm này!", threadID, messageID);
		...
		// Unban user
		if (contentMessage.indexOf(`${prefix}un`) == 0 && admins.includes(senderID)) {
			...
			mentions.forEach(mention => {
				...
				User.unban(mention).then(success => {
					...
					api.sendMessage({
						body: `Đã giải thoát cho thằng ngu này!`,
						mentions: [{
							tag: 'Đã giải thoát cho thằng ngu này!',
							id: mention
						}]
					}, threadID, messageID);
		...
		// Ban thread
		if (contentMessage == `${prefix}banbox` && admins.includes(senderID)) {
			Thread.ban(parseInt(threadID)).then((success) => {
				if (!success) return api.sendMessage("Không thể ban group này!", threadID, messageID);
				api.sendMessage("Nhóm này đã bị chặn, sẽ không thể sử dụng lệnh của bot nữa!", threadID, messageID);
		...
		if (contentMessage.indexOf(`${prefix}ban`) == 0 && admins.includes(senderID)) {
			...
			mentions.forEach(mention => {
				...
				User.ban(parseInt(mention)).then((success) => {
					...
					api.sendMessage({
						body: `Thằng lồn nào đó vừa ăn ban!`,
						mentions: [{
							tag: 'Thằng lồn nào đó vừa ăn ban!',
							id: parseInt(mention)
						}]
					}, threadID, messageID);
		...
		if (contentMessage.indexOf(`${prefix}báo lỗi`) == 0) {
		...
		//restart
		if (contentMessage == `${prefix}load` && admins.includes(senderID)) return api.sendMessage(`Đang làm mới hoạt động bot, đợi xíu nhen <33`, threadID, () => require("node-cmd").run("pm2 restart 0"), messageID);
	...
	/* ==================== Help Commands ================*/
		...
		if (contentMessage.indexOf(`${prefix}help`) == 0) {
			...
			if (content.length == 0) {
				var helpGroup = [];
				var helpMsg = "ĐÂY LÀ TẤT CẢ LỆNH CÓ TRONG BOT:\n\n";
		...
		if (contentMessage.indexOf(`${prefix}yêu cầu`) == 0) {
			var content = contentMessage.slice(prefix.length + 8,contentMessage.length);
			...
			if (content.indexOf("thêm") == 0) {
				var addnew = content.slice(5, content.length);
			...
			else if (content.indexOf("xoá") == 0 && admins.includes(senderID)) {
	...
	/* ==================== Media Commands ==================== */
		...
		//poke
		if (contentMessage.indexOf(`${prefix}chọc`) == 0 && contentMessage.indexOf('@') !== -1)
			...
			//pat
		if (contentMessage.indexOf(`${prefix}xoa`) == 0 && contentMessage.indexOf('@') !== -1)
			return request('https://nekos.life/api/v2/img/pat', (err, response, body) =>{
				let picData = JSON.parse(body);
				let getURL = picData.url;
				let ext = getURL.substring(getURL.lastIndexOf(".") + 1);
				let tag = contentMessage.slice(prefix.length + 4, contentMessage.length).replace("@", "");
				let callback = function() {
					api.sendMessage({
						body: tag + ", I pat you ❤️",
						mentions: [{
							tag: tag,
							id: Object.keys(event.mentions)[0]
						}],
		...
		//hug
		if (contentMessage.indexOf(`${prefix}ôm`) == 0 && contentMessage.indexOf('@') !== -1)
		...
		//kiss
		if (contentMessage.indexOf(`${prefix}hôn`) == 0 && contentMessage.indexOf('@') !== -1)
		...
		//tát
		if (contentMessage.indexOf(`${prefix}vả`) == 0 && contentMessage.indexOf('@') !== -1)
	...
	/* ==================== General Commands ================*/
		...
		//giveaway
		if (contentMessage.indexOf(`${prefix}game`) == 0) {
			var content = contentMessage.slice(prefix.length + 5, contentMessage.length);
		...
		//chọn
		if (contentMessage.indexOf(`${prefix}chọn`) == 0) {
			var input = contentMessage.slice(prefix.length + 5, contentMessage.length).trim();
	...
	/* ==================== NSFW Commands ==================== */
		...
		else if (contentMessage.indexOf(`${prefix}setmoney`) == 0) return api.sendMessage("Bạn không phải là admin, không có quyền sử dụng lệnh này!", threadID, messageID);
		...
		if (contentMessage.indexOf(`${prefix}fishing`) == 0)
			...
			//test
			if (contentMessage == 'doan' && admins.includes(senderID)) { 
			return api.sendMessage({
						body: `Goshujin-sama gọi gì tớ thế?`,
						mentions: [{
							tag: 'Goshujin-sama',
							id: senderID
						}]
			...
			if (contentMessage == 'doan') { 
			return api.sendMessage({
						body: `Bạn gọi gì mình thế?`,
						mentions: [{
							tag: 'Bạn',
							id: senderID
						}]
		...
		if (contentMessage == `${prefix}top rank`)
			(async () => {
				let msg = {
				body: 'Top 10 Người Có Bật Xếp Hạng Cao Nhất Trên Toàn Box Chat:',
				mentions: []
				}
				let num = 0;
			...
			//Global Money
			if (contentMessage == `${prefix}top money`)
          (async () => {
            let num = 0;
            let all = await User.getColumn(['name', 'uid', 'money']);
            all.sort((a, b) => b.money - a.money);
            let msg = {
              body: `Top 5 Người Có Số Tiền Cao Nhất Trên Toàn Box Chat`,
              mentions: []
            } 
            for (var i = 0; i < 5; i++) {
              num += 1;
			...
			//name
			if (contentMessage == `${prefix}name`) return api.sendMessage(`Tên tôi là: ${config.botName}`, threadID, messageID);
			...
			//reaction
			if (contentMessage =='yêu' && (yêu.bestMatch.rating >= 0.3)) return api.setMessageReaction("\uD83D\uDE0D", messageID);
			
			 //haha
    if (contentMessage =='Haha') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage =='Ahaha') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage =='Adu vjp') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage ==':))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage ==':)') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage ==':)))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage ==':))))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage ==':))))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    ...
    if (contentMessage ==':)))))))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage ==':)))))))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage =='Cac') return api.setMessageReaction("\uD83D\uDE06", messageID);
    if (contentMessage =='÷))') return api.setMessageReaction("\uD83D\uDE06", messageID);
    //love
    if (contentMessage =='Xinh') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage =='Đẹp') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage ==':3') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage ==':>') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage =='uwu') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage =='._.') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage =='.-.') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage =='-_-') return api.setMessageReaction("\uD83D\uDC97", messageID);
    if (contentMessage ==':<') return api.setMessageReaction("\uD83D\uDC97", messageID);
    //headlove
    if (contentMessage =='Yêu') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='Ok') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage ==':D?') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage ==':D??') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage ==':Đ') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='Love') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='Love you') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='xD') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='XD') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='Vjp') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='Noice') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='Nice') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='👏') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='🤣🤣🤣') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='I love you') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    if (contentMessage =='UwU') return api.setMessageReaction("\uD83D\uDE0D", messageID);
    //buon
    if (contentMessage =='Vcl') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='Hic') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==';v') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='Khóc') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==':((') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==':(') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='😢') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==':v') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==';V') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='Bủh') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='Bruh') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==':V') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='Dude') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage ==':<') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='!work') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='=(((') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='!fishing') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='!daily') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='!money') return api.setMessageReaction("\uD83D\uDE22", messageID);
    if (contentMessage =='!penis') return api.setMessageReaction("\uD83D\uDE22", messageID);
    //wow
    if (contentMessage =='Adu') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Gì thế') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Đù') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Ghê') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage ==':()') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Vãi nồi') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Thú vị') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Hay phết') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Vjp bro') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage =='Bro') return api.setMessageReaction("\uD83D\uDE2E", messageID);
    if (contentMessage ==':[]') return api.setMessageReaction("\uD83D\uDE2E", messageID);
			
			//care
			if (contentMessage == `${prefix}care`) return api.sendMessage(`⊂_ヽ
     ＼＼   don't
         ＼( ͡° ͜ʖ ͡°)
              >    ⌒ヽ
            /      へ＼
